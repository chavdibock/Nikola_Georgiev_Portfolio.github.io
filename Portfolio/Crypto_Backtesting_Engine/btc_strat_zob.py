...
path = kagglehub.dataset_download("novandraanugrah/solana-historical-datasets-2018-2024")
...
# --- Target Variable (AFTER dropping feature NaNs) ---
...
# --- Rebased Performance Plot (Test Only) ---
test_signals['BuyHold'] = test_signals['BuyHold'] / test_signals['BuyHold'].iloc[0]
test_signals['CumulativeReturns'] = test_signals['CumulativeReturns'] / test_signals['CumulativeReturns'].iloc[0]

plt.figure(figsize=(12, 6))
plt.title("Out-of-Sample Strategy Performance (Test Period)")
plt.plot(test_signals['CumulativeReturns'], label='ML Strategy')
plt.plot(test_signals['BuyHold'], label='Buy & Hold', alpha=0.6)
